// ═══════════════════════════════════════════════════════════
//  DR. SHROOM — B2B INVOICE APP BACKEND (additions)
//  Paste this entire file's contents into your EXISTING
//  Apps Script project (the one with doGet/doPost for orders).
//  Do NOT delete your existing doGet/doPost — see merge notes
...
// ═══════════════════════════════════════════════════════════
//  ROUTER — merge into your existing doGet / doPost
// ═══════════════════════════════════════════════════════════
//
// Your existing doGet(e) handles ?orderId=... for order tracking.
// Add this as the FIRST check inside your existing doGet, so the
// invoice app can also use GET for read-only data:
//
//   function doGet(e) {
//     const action = e.parameter.action;
//     if (action) return routeInvoiceGet(action, e);
//     ... your existing orderId tracking code stays below ...
//   }
//
// Your existing doPost(e) parses e.postData.contents as the order
// payload. Add this as the FIRST check inside your existing doPost,
// right after JSON.parse, so invoice actions don't fall through into
...
//       ... your existing order-creation code stays below ...
//     } finally { lock.releaseLock(); }
//   }
//
// ═══════════════════════════════════════════════════════════

function routeInvoiceGet(action, e) {
  try {
    switch (action) {
      ...
      default:                return buildResponse({ success: false, error: 'Unknown action: ' + action });
    }
  } catch (err) {
...
function routeInvoicePost(data) {
  try {
    switch (data.action) {
      ...
      default:                   return buildResponse({ success: false, error: 'Unknown action: ' + data.action });
    }
  } catch (err) {
...
function getInvoiceSheet_() {
  return getOrCreateSheet_(INV_SHEET,
    ['Invoice No', 'Date', 'Financial Year', 'Recipient Name', 'Recipient Address', 'Recipient Phone', 'Items (JSON)', 'Total', 'Created By']);
}
